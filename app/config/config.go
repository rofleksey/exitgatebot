...
type Config struct {
	...
	ServiceName string    `yaml:"service_name" env:"SERVICE_NAME" example:"exitgatebot" validate:"required"`
	...
	Steam       Steam     `yaml:"steam" envPrefix:"STEAM_"`
	OpenAI      OpenAI    `yaml:"openai" envPrefix:"OPENAI_"`
	Profiles    []Profile `yaml:"profiles" envPrefix:"PROFILES_" validate:"required"`
}
...
type Steam struct {
	...
	CheckInterval int `yaml:"check_interval" example:"10" validate:"required"`
}

type OpenAI struct {
	// OpenAI base url
	BaseURL string `yaml:"base_url" example:"https://bothub.chat/api/v2/openai/v1" validate:"required"`
	// OpenAI token
	Token string `yaml:"token" example:"sk-proj-abc123456789DEF789ghi012JKL345mno678PQR901stu234VWX" validate:"required"`
	// OpenAI model
	Model string `yaml:"model" example:"deepseek-v3.2-exp" validate:"required"`
}

type Profile struct {
	// Link to steam profile
	SteamLink string `yaml:"steam_link" example:"https://steamcommunity.com/id/Maznevich" validate:"required"`
	// Twitch username (lowercase)
	Username string `yaml:"username" example:"maznevich" validate:"required"`
}
...
func Load(configPath string) (*Config, error) {
	...
	if result.ServiceName == "" {
		result.ServiceName = "exitgatebot"
	}
